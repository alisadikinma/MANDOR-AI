...
> **Product label vs. entity name — `issue` is surfaced as "Ticket".** The user-facing product label for an `issue` is **"Ticket"** (zh **工单**, ja **チケット**, ko **티켓**) — "issue" reads as "problem" to end users, so the UI calls it a Ticket. This is a **display-copy decision only**: the entity name everywhere in code, DB columns, API fields, TS types, and routes (`/{slug}/issues`) stays `issue`. So a string in `packages/views/locales/*` says "Ticket", but `issue_status`, `IssueRow`, `paths.issues()`, and the `issue` table are unchanged. When you see "Ticket" in the UI and `issue` in the code, that is intentional, not drift.
